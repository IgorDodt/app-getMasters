...
export class ServiceProviderController extends BaseController<ServiceProvider> {
    ...
    private validationDefault(_serviceProvider: ServiceProvider) {
    ...
    async createServiceProvider(request: Request) {
        ...
        super.isRequired(_serviceProvider.password, 'Informe a senha!');
        super.isRequired(confirmPassword, 'A confirmação de senha é obrigatório');
